...
import { getLang, getPrefixLang, splitLang } from '../../selectors/lang'
import { ButtonBase, Tooltip, Typography, withStyles } from '@material-ui/core'
import { useActiveOptions } from '../../hooks'
import { getGapDemographics, isGapDemographic } from '../../selectors'
...
const getTooltipLang = (type, value) => {
  ...
  const gapDems = isGap ? getGapDemographics(value): [value, value]
  switch (type) {
    case "metric":
      return getPrefixLang(value, "HINT")
    case "demographic":
      return getLang(isGap ? 'HINT_GAP_DEMOGRAPHIC' : 'HINT_DEMOGRAPHIC', {
...
const SedaHeaderTitle = ({ classes, className, ...props }) => {
  ...
  const titleContext = { opportunityType: getPrefixLang(metricId, `LABEL_CONCEPT`), filterLocation }
  // get title string
  ...
  const subtitleComponents = {
    region: <SedaOptionButton className={classes.button} type="region" value={regionId} />,
    demographic: <SedaOptionButton className={classes.button} type="demographic" value={demId} />,
    metric: <SedaOptionButton className={classes.button} type="metric" value={metricId} />
  }
  // get subtitle string
  ...
  return (
    <div className={clsx(classes.root, className)} {...props}>
      ...
      <Typography className={classes.subtitle} variant="body2">
        <InterpolateComponents text={subtitle} components={subtitleComponents} />
      </Typography>
    </div>
  )
}
